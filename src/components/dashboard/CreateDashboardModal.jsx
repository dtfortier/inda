...
/* ── Step indicator (left stepper) ────────────────────────── */

const STEPS = [
  { num: 1, label: 'Dashboard details', desc: 'Name your dashboard and choose who it\'s for.' },
  { num: 2, label: 'Define scope',       desc: 'Choose the data this dashboard will include.' },
  { num: 3, label: 'Review & create',   desc: 'Preview and confirm your dashboard settings.' },
]

function Stepper({ current }) {
  return (
    <div className="cdm-stepper">
      <ul className="cdm-stepper-list">
        {STEPS.map(s => {
          const state = s.num < current ? 'done' : s.num === current ? 'active' : 'upcoming'
          return (
            <li key={s.num} className={`cdm-step cdm-step--${state}`}>
              <div className="cdm-step-indicator">
                {state === 'done'
                  ? <span className="cdm-step-num cdm-step-num--done"><CheckIcon /></span>
                  : <span className="cdm-step-num">{s.num}</span>
                }
                {s.num < STEPS.length && <div className="cdm-step-line" />}
              </div>
              <div className="cdm-step-text">
                <div className="cdm-step-label">{s.label}</div>
                <div className="cdm-step-desc">{s.desc}</div>
              </div>
            </li>
          )
        })}
      ...
      <div className="cdm-stepper-callout">
        <span className="cdm-stepper-callout-icon"><LockIcon /></span>
        <div>
          <div className="cdm-stepper-callout-title">Permissions-aware</div>
          <div className="cdm-stepper-callout-body">
            Scope options are filtered based on the selected audience's role and data permissions.
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="cdm-audience">
      ...
      <div className="cdm-audience-card">
        <div className="cdm-audience-avatar">
          {isSelf ? 'You' : audience.label.split(' ').map(w => w[0]).join('').slice(0, 2)}
        </div>
        ...
        {PERMISSION_ROWS.map(row => {
          ...
          return (
            <li key={row.key} className={`cdm-audience-perm-row${isNone ? ' cdm-audience-perm-row--none' : ''}`}>
              ...
              <button className="cdm-audience-perm-info" aria-label={`Info about ${row.label}`}>
                <InfoIcon />
              </button>
            </li>
          )
...
function StepDetails({ form, onChange, audience, onAudienceChange }) {
  ...
  return (
    <div className="cdm-step-content">
      <h2 className="cdm-step-heading">1. Dashboard details</h2>
      ...
      {/* Description */}
      <div className="cdm-field">
        <label className="cdm-label" htmlFor="db-desc">Description <span className="cdm-optional">(optional)</span></label>
        <textarea
          id="db-desc"
      ...
      <div className="cdm-field">
        <label className="cdm-label">
          Dashboard for
          <button className="cdm-info-inline" aria-label="About dashboard audience">
            <InfoIcon />
          </button>
        </label>

        <div className="cdm-for-row">
          {/* Audience dropdown */}
          <div className="cdm-dropdown-wrap" ref={dropRef}>
            ...
            >
              <span className="cdm-dropdown-avatar">
                {audience.id === 'myself' ? 'Me' : audience.label.split(' ').map(w => w[0]).join('').slice(0, 2)}
              </span>
              <span className="cdm-dropdown-label">{audience.id === 'myself' ? 'Myself' : audience.label}</span>
        ...
        {/* Permissions note */}
        <div className="cdm-permissions-note">
          <InfoIcon />
          {isSelf
      ...
      {/* Next button */}
      <div className="cdm-step-footer-inline">
        <button
          className="cdm-btn cdm-btn--primary"
          disabled={!form.name.trim()}
          onClick={() => onChange({ _nextStep: 2 })}
        >
          Next: Define scope <ArrowRightIcon />
        </button>
      </div>
    </div>
  )
...
/* ── Step 2: Define scope (simplified — same pattern, no inspector for now) ── */
...
const SAMPLE_OPTIONS = {
  subAccounts:   [{ value: 'College of Business', meta: '142 courses' }, { value: 'College of Science', meta: '98 courses' }, { value: 'College of Arts', meta: '74 courses' }, { value: 'College of Education', meta: '55 courses' }, { value: 'School of Engineering', meta: '120 courses' }],
  terms:         [{ value: 'Spring 2025' }, { value: 'Fall 2024' }, { value: 'Winter 2025' }],
  studentGroups: [{ value: 'First-generation Students', meta: '892 students' }, { value: 'Transfer Students', meta: '341 students' }, { value: 'Student Athletes', meta: '215 students' }, { value: 'Students on Probation', meta: '128 students' }, { value: 'International Students', meta: '476 students' }, { value: 'Biology Majors', meta: '203 students' }, { value: 'Undecided/Exploratory', meta: '312 students' }, { value: 'Honor Students', meta: '189 students' }],
  courses:       [{ value: 'BIO 101', meta: '28 students' }, { value: 'CHEM 201', meta: '22 students' }, { value: 'ENG 110', meta: '35 students' }, { value: 'MATH 150', meta: '30 students' }],
  courseGroups:  [{ value: 'Independent Study', meta: '12 courses' }, { value: 'Pre-Registration Eligible', meta: '45 courses' }, { value: 'South Campus', meta: '28 courses' }, { value: 'Cross-Listed Courses', meta: '19 courses' }],
  instructors:   [{ value: 'Dr. Emily Carter' }, { value: 'Prof. James Liu' }, { value: 'Dr. Maria Santos' }, { value: 'Dr. Kevin Osei' }],
}
...
function ScopeField({ fieldDef, selected, onAdd, onRemove, audiencePerms }) {
  ...
  const allOptions = SAMPLE_OPTIONS[fieldDef.key] || []
  const available = allOptions.filter(o => !selected.includes(o.value))

  // Determine if this field is restricted for the audience
  ...
  return (
    <div className={`cdm-scope-field${isNone ? ' cdm-scope-field--disabled' : ''}`}>
      <div className="cdm-scope-field-header">
        <div className="cdm-scope-field-label">
          ...
          <div className="cdm-scope-info-wrap">
            <button
              className="cdm-scope-info-btn"
              onClick={() => setInfoOpen(o => !o)}
              aria-label={`Info about ${fieldDef.label}`}
            >
              <InfoIcon />
            </button>
            {infoOpen && (
              <div className="cdm-scope-info-tooltip">{fieldDef.description}</div>
            )}
          </div>
        ...
        <div className="cdm-scope-chips">
          {selected.map(val => (
            <span key={val} className="cdm-scope-chip">
              <span>{val}</span>
              <button
                className="cdm-scope-chip-remove"
                onClick={() => onRemove(fieldDef.key, val)}
          ...
          <div className="cdm-scope-add-wrap" ref={addRef}>
            ...
            {popoverOpen && (
              <div className="cdm-scope-add-menu">
                {available.length === 0 ? (
                  <div className="cdm-scope-add-empty">All options selected</div>
                ) : available.map(opt => (
                  <button
                    key={opt.value}
                    className="cdm-scope-add-item"
                    onClick={() => { onAdd(fieldDef.key, opt.value); setPopoverOpen(false) }}
                  >
      ...
      {isNone && (
        <p className="cdm-scope-none-msg">
          This audience doesn't have access to {fieldDef.label.toLowerCase()} data.
        </p>
      )}
    </div>
  )
...
function StepScope({ scope, onScopeChange, audience }) {
  ...
  const handleAdd = (key, val) => {
    onScopeChange({ ...scope, [key]: [...(scope[key] || []), val] })
  }
  const handleRemove = (key, val) => {
    onScopeChange({ ...scope, [key]: (scope[key] || []).filter(v => v !== val) })
  }

  return (
    <div className="cdm-step-content">
      <h2 className="cdm-step-heading">2. Define scope</h2>
      <p className="cdm-step-subtext">
        Select the data this dashboard will include. Filters apply within{' '}
        {audience.id === 'myself' ? 'your' : `${audience.label}'s`} access permissions.
      </p>
...
function StepReview({ form, audience, scope }) {
  const isSelf = audience.id === 'myself'
  const scopeEntries = SCOPE_FIELDS.map(f => ({
    label: f.label,
    values: scope[f.key] || [],
  })).filter(e => e.values.length > 0)

  const hasLimitedPerms = !isSelf && Object.values(audience.permissions).some(
    p => p.label === 'None' || p.label === 'Limited'
  )

  return (
    <div className="cdm-step-content">
      <h2 className="cdm-step-heading">3. Review &amp; create</h2>
      ...
      {hasLimitedPerms && (
        <div className="cdm-review-notice">
          <InfoIcon />
          <span>
            Some scope categories are unavailable due to {audience.label}'s permission level.
            The dashboard will only include data within their access.
          </span>
        </div>
      )}
            ...
            {scopeEntries.map(e => (
              <div key={e.label} className="cdm-review-scope-entry">
                ...
                <div className="cdm-review-scope-chips">
                  {e.values.map(v => (
                    <span key={v} className="cdm-review-chip">{v}</span>
                  ))}
                </div>
              </div>
            ))}
...
function BottomBar({ step, onBack, onCancel, onSaveExit, onNext, onCreate, canProceed, creating }) {
  return (
    <div className="cdm-bottom-bar">
      ...
      <div className="cdm-bottom-steps">
        {STEPS.map(s => (
          <span key={s.num} className={`cdm-bottom-step${s.num === step ? ' active' : s.num < step ? ' done' : ''}`}>
      ...
      <div className="cdm-bottom-actions">
        {step > 1 && (
          <button className="cdm-btn cdm-btn--tertiary" onClick={onBack}>Back</button>
        )}
...
export default function CreateDashboardModal({ isOpen, onClose, onCreate }) {
  ...
  const handleFormChange = (patch) => {
    if (patch._nextStep) { setStep(patch._nextStep); return }
  ...
  return (
    <div className="cdm-backdrop" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="cdm-modal" role="dialog" aria-modal="true" aria-label="Create a new dashboard">
        ...
        {/* Three-column body: stepper | content | audience */}
        <div className="cdm-body">
          ...
          <div className="cdm-content">
            ...
            {step === 3 && (
              <StepReview form={form} audience={audience} scope={scope} />
            )}
          </div>
